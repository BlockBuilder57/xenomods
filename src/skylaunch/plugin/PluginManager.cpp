...
#include "skylaunch/utils/utils.h"
...
namespace plugin {
    ...
    void Manager::LoadPluginsImpl() {
		...
		skylaunch::logger::s_Instance->LogFormat("[PluginManager] Initializing plugins...");
		...
		skylaunch::utils::walkDirectory(utils::g_RomMountStr + PLUGIN_PATH,
        ...
        if (m_pluginInfos.empty()) {
			skylaunch::logger::s_Instance->LogFormat("[PluginManager] No plugin to load.");
            return;
        }

        // open plugins
		skylaunch::logger::s_Instance->LogFormat("[PluginManager] Opening plugins...");
        ...
        while (pluginInfoIter != m_pluginInfos.end()) {
            ...
            if (R_FAILED(rc)) {
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Failed to open '%s' (0x%x). Skipping.",
            ...
            if (R_FAILED(rc)) {
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Failed to get '%s' size. (0x%x). Skipping.",
                                                       plugin.Path.c_str(), rc);
                // stop tracking
                pluginInfoIter = m_pluginInfos.erase(pluginInfoIter);
                continue;
            ...
            rc = skylaunch::utils::readFile(plugin.Path, 0, plugin.Data.get(), plugin.Size);
            if (R_SUCCEEDED(rc))
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Read %s", plugin.Path.c_str());
            else {
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Failed to read '%s'. (0x%x). Skipping.",
                                                       plugin.Path.c_str(), rc);
                // stop tracking
                pluginInfoIter = m_pluginInfos.erase(pluginInfoIter);
                continue;
            ...
            if (sortedHashes.find(plugin.Hash) != sortedHashes.end()) {
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] '%s' is detected duplicate, Skipping.",
                                                       plugin.Path.c_str());
                // stop tracking
                pluginInfoIter = m_pluginInfos.erase(pluginInfoIter);
                continue;
		...
		skylaunch::logger::s_Instance->LogFormat("[PluginManager] NRR size: %d", m_nrrSize);
        ...
        *nrrHeader = nn::ro::NrrHeader{
        ...
        if (R_FAILED(rc)) {
			...
			skylaunch::logger::s_Instance->LogFormat("[PluginManager] Failed to register NRR (0x%x).", rc);
            return;
        }

        // load plugins

		skylaunch::logger::s_Instance->Log("[PluginManager] Loading plugins...\n");
        ...
        while (pluginInfoIter != m_pluginInfos.end()) {
            ...
            if (R_SUCCEEDED(rc)) {
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Loaded '%s'", plugin.Path.c_str(),
                                                       &plugin.Module.Name);
            } else {
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Failed to load '%s' (0x%x). Skipping.",
                                                       plugin.Path.c_str(), rc);
                // stop tracking
                pluginInfoIter = m_pluginInfos.erase(pluginInfoIter);
                continue;
        ...
        while (pluginInfoIter != m_pluginInfos.end()) {
			...
			skylaunch::logger::s_Instance->LogFormat("[PluginManager] Running `main` for %s", plugin.Path.c_str(),
                                                   &plugin.Module.Name);
            ...
            if (pluginEntrypoint != NULL && R_SUCCEEDED(rc)) {
                pluginEntrypoint();
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Finished running `main` for '%s' (0x%x)",
                                                       plugin.Path.c_str(), rc);
            } else {
				skylaunch::logger::s_Instance->LogFormat("[PluginManager] Failed to lookup symbol for '%s' (0x%x)",
                                                       plugin.Path.c_str(), rc);
            }
...
void get_plugin_addresses(const void* internal_addr, void** start, void** end) {
    auto info = skylaunch::plugin::Manager::GetContainingPlugin(internal_addr);
    if (info == nullptr)
        *start = *end = nullptr;
    else {
