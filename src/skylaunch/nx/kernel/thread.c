...
#include "skylaunch/nx/kernel/thread.h"
...
Result threadCreate(Thread* t, ThreadFunc entry, void* arg, void* stack, size_t stack_sz, int prio, int cpuid) {
    ...
    if (stack == NULL) stack = memalign(0x1000, stack_sz + reent_sz + tls_sz);

    if (stack == NULL) {
        rc = MAKERESULT(Module_Libnx, LibnxError_OutOfMemory);
    } else {
        void* stack_mirror = virtmemReserve(stack_sz);
        ...
        if (R_SUCCEEDED(rc)) {
            ...
            rc = svcCreateThread(&handle, (ThreadFunc)&_EntryWrap, args, (void*)stack_top, prio, cpuid);
            ...
            if (R_SUCCEEDED(rc)) {
                ...
                if (tls_load_sz) memcpy(args->tls, __tdata_lma, tls_load_sz);
                if (tls_bss_sz) memset(args->tls + tls_load_sz, 0, tls_bss_sz);
            }
        }
    }
